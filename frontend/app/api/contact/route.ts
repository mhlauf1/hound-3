...
async function verifyRecaptcha(token: unknown): Promise<boolean> {
  ...
  try {
    ...
    console.log('[contact] reCAPTCHA verify:', data)
  ...
  try {
    ...
    if (!(await verifyRecaptcha(recaptchaToken))) {
      return NextResponse.json(
        {error: 'Verification failed. Please try again, or call us at 651-788-9797.'},
        {status: 400},
      )
    }

    // Local testing: verify reCAPTCHA but skip the actual email send
    if (process.env.CONTACT_FORM_DRY_RUN === 'true') {
      console.log('[contact] DRY RUN — email not sent. Fields:', fields)
