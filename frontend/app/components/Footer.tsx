...
export default function Footer({tagline, columns, contactInfo, footerText}: FooterProps) {
  return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div>
            <div className="mb-4">
              ...
              <div className="font-sans text-[11px] font-medium uppercase tracking-[0.2em]">Resort</div>
            </div>
            {tagline && (
              <p className="font-sans text-[15px] text-text-muted leading-relaxed">{tagline}</p>
            )}
          </div>
          ...
          {contactInfo && (
            <div>
              ...
              <div className="space-y-3 font-sans text-[15px] text-text-muted">
                ...
                {contactInfo.email && (
                  <a href={`mailto:${contactInfo.email}`} className="block hover:text-dark transition-colors">
                    {contactInfo.email}
                  </a>
                )}
              </div>
      ...
      <div className="border-t border-border-light">
        <div className="container py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="font-sans text-[14px] text-text-muted">
            {footerText || `\u00A9 ${new Date().getFullYear()} Hound Around Resort. All rights reserved.`}
          </p>
          <div className="flex items-center gap-6">
            <Link href="/privacy" className="font-sans text-[14px] text-text-muted hover:text-dark transition-colors">
              Privacy Policy
            </Link>
            <Link href="/terms" className="font-sans text-[14px] text-text-muted hover:text-dark transition-colors">
              Terms of Service
            </Link>
          </div>
        </div>
